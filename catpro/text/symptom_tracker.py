...
# Based on https://github.com/MaartenGr/KeyBERT/blob/master/keybert/_model.py
...
class KeyBERT:
	...
	def extract_keywords(self,
	                     ...
	                     seed_lexicon: List[str] = None,
	                     gen_keywords:bool = False,
	                     seed_weights: List[float] = None) -> Union[List[Tuple[str, float]],
		...
		Arguments:
			...
			seed_lexicon: Seed keywords that may guide the extraction of keywords by
	...
	def _extract_keywords_single_doc(self,
	                                 ...
	                                 vectorizer: CountVectorizer = None,
	                                 seed_lexicon: List[str] = None,
	                                 gen_seed_lexicon: bool = False,
	                                 seed_weights: List[float] = None) -> List[Tuple[str, float]]:
		...
		try:
			...
			if candidates is None:
				if vectorizer:
					...
					if keyphrase_ngram_range == 'automatic':
						seed_words = [len(n.split()) for n in seed_lexicon]
						keyphrase_ngram_range = (int(np.min(seed_words)), int(np.max(seed_words)))
			...
			# TODO: obtain keywords from Reddit, wikipedia, empath,


			# if gen_seed_lexicon:
				#
				# seed_lexicon.append(new_keywords)
			# if seed_lexicon is not None:

			# todo, have option for weighting seed words:
			# weighting: wieighted average or sum of embeddings

			if seed_weights:
				seed_embeddings = self.model.embed(seed_lexicon) #list
				seed_embeddings = np.average([seed_embeddings], axis=0, weights=seed_weights) #seed_weights = [1,2,1,1,3,3,2]
				# todo: try option where you take the averahe without weights and compare to " ".join(seed_lexicon)
			else:
				# seed_embeddings = self.model.embed([" ".join(seed_lexicon)])
				seed_embeddings = self.model.embed(seed_lexicon)
			...
			# 	# keywords = max_sum_similarity(seed_embeddings, candidate_embeddings, candidates, top_n, nr_candidates)
			#
			# else:
			distances = cosine_similarity(seed_embeddings, candidate_embeddings)

			top_n_most_similar = [(candidates[index], round(float(distances[0][index]), 4))
			            for index in distances.argsort()[0][-top_n:]][::-1]

			return distances, candidates, top_n_most_similar, keyphrase_ngram_range
		except ValueError:
			return []
